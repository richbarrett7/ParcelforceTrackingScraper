...
class ParcelforceTrackingScraper {
    ...
    function getEvents() {
        ...
        preg_match_all('/signed for by (.+)<span/', $this->response['data'], $signature_name);
        $signature_name = $signature_name[1];

        preg_match_all('/<img [>]*src="data:([^"]*)"/i', $this->response['data'],$signature_image);
